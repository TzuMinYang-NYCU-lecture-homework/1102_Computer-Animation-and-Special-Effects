...
void Cloth::initializeSpring() {
  ...
  // Write code here!
  /// index的用途是標記彈簧兩端的particle是誰
  /// 直的struct
  for (int i = 0; i < particlesPerEdge - 1; ++i) {  /// row
    for (int j = 0; j < particlesPerEdge; ++j) {    /// col
      int index = i * particlesPerEdge + j;
  ...
  /// 左上往右下的bending
  float bendingLength = (_particles.position(0) - _particles.position(particlesPerEdge + 1)).norm();
  for (int i = 0; i < particlesPerEdge - 1; ++i) {  /// row
    for (int j = 0; j < particlesPerEdge - 1; ++j) {    /// col
      int index = i * particlesPerEdge + j;
      _springs.emplace_back(index, index + particlesPerEdge + 1, bendingLength, Spring::Type::BEND);
    }
  }

  /// 右上往左下的bending
  for (int i = 0; i < particlesPerEdge - 1; ++i) {    /// row
    for (int j = 1; j < particlesPerEdge; ++j) {      /// col
      int index = i * particlesPerEdge + j;
      _springs.emplace_back(index, index + particlesPerEdge - 1, bendingLength, Spring::Type::BEND);
    }
  }
  ...
  /// 橫的shear
  float shearLength = (_particles.position(0) - _particles.position(2)).norm();
  for (int i = 0; i < particlesPerEdge; ++i) {          /// row
    for (int j = 0; j < particlesPerEdge - 2; ++j) {    /// col
      int index = i * particlesPerEdge + j;
      _springs.emplace_back(index, index + 2, shearLength, Spring::Type::SHEAR);
    }
  }

  /// 直的shear
  for (int i = 0; i < particlesPerEdge - 2; ++i) {    /// row
    for (int j = 0; j < particlesPerEdge; ++j) {      /// col
      int index = i * particlesPerEdge + j;
      _springs.emplace_back(index, index + particlesPerEdge + particlesPerEdge, shearLength, Spring::Type::SHEAR);
    }
  }

  //printf("%d\n", _springs.size()); //3502是對的
...
void Cloth::computeSpringForce() {
  ...
  for (const auto& spring : _springs) {
    ...
    /// for a
    // Eigen::Vector4f用auto在release版本會有問題，可看Eigen(pitfalls)
    Eigen::Vector4f position_vector_ab = (_particles.position(spring.startParticleIndex()) - _particles.position(spring.endParticleIndex()));
    auto position_vector_ab_norm = position_vector_ab.norm();
    Eigen::Vector4f spring_force_a = -springCoef * (position_vector_ab_norm - spring.length()) * (position_vector_ab / position_vector_ab_norm);

    Eigen::Vector4f velocity_vector_ab = (_particles.velocity(spring.startParticleIndex()) - _particles.velocity(spring.endParticleIndex()));
    Eigen::Vector4f damping_force_a = -damperCoef * (velocity_vector_ab.dot(position_vector_ab) / position_vector_ab_norm) * (position_vector_ab / position_vector_ab_norm);
    
    Eigen::Vector4f force_a = spring_force_a + damping_force_a;
    _particles.acceleration(spring.startParticleIndex()) += force_a * _particles.inverseMass(spring.startParticleIndex());

    /// for b
    Eigen::Vector4f position_vector_ba = -position_vector_ab;
    auto position_vector_ba_norm = position_vector_ab_norm;
    Eigen::Vector4f spring_force_b = -springCoef * (position_vector_ba_norm - spring.length()) * (position_vector_ba / position_vector_ba_norm);

    Eigen::Vector4f velocity_vector_ba = -velocity_vector_ab;
    Eigen::Vector4f damping_force_b = -damperCoef * (velocity_vector_ba.dot(position_vector_ba) / position_vector_ba_norm) * (position_vector_ba / position_vector_ba_norm);
    
    Eigen::Vector4f force_b = spring_force_b + damping_force_b;
    _particles.acceleration(spring.endParticleIndex()) += force_b * _particles.inverseMass(spring.endParticleIndex());
    /// end of my code
  }
}
